...
func (r *Repository) Create(ctx context.Context, reader models.Reader) error {
	...
	_, err := r.db.Exec(ctx, query, reader.Name, reader.PhoneNumber, reader.Address, reader.DateOfBirth)
	if err != nil {
...
func (r *Repository) GetIdByName(ctx context.Context, name string) (int, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return 0, fmt.Errorf("[repo][GetIdByName] %w", constants.ErrReaderNotFound)
		}
...
func (r *Repository) Delete(ctx context.Context, id int) error {
	...
	if cmdTag.RowsAffected() == 0 {
		return fmt.Errorf("[repo][Delete] %w", constants.ErrReaderNotFound)
	}
	return nil
}
...
func (r *Repository) UpdateContactInfo(ctx context.Context, readerId int, phoneNumber string, address string) error {
	...
	if cmdTag.RowsAffected() == 0 {
		return fmt.Errorf("[repo][UpdateContactInfo] читатель с ID %d: %w", readerId, constants.ErrReaderNotFound)
	}
	return nil
}
